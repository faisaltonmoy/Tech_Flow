...
if (isset($_GET['id']) && $_GET['id'] != '') {
    ...
    $add_sql = "select * from news where id = '$id'";
    $res = mysqli_query($con, $add_sql);

    $ck = mysqli_num_rows($res);
    if ($ck > 0) {
        $row = mysqli_fetch_assoc($res);
        $catagories = $row['message'];
    } else {
...
if (isset($_POST['submit'])) {
    $catagories = get_safe_val($con, $_POST['message']);
    $add_sql = "select * from news where message = '$catagories'";
    $res = mysqli_query($con, $add_sql);
    ...
    if ($msg == '') {
        if (isset($_GET['id']) && $_GET['id'] != '') {
            mysqli_query($con, "update news set message = '$catagories' where id = '$id'");
        } else {
            $add_sql = "insert into news(message,status) values('$catagories','0')";
            $res = mysqli_query($con, $add_sql);
        }
        header('location:news.php');
        die();
